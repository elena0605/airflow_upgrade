# /opt/airflow/dags/tiktok_batch_openai_dag.py
...
PROMPT = os.getenv("TIKTOK_PROMPT") or '''Task: You are an expert in media psychology, visual communication, and public-health behaviour. Analyze the thumbnail image and output ONLY valid JSON with the following fields:
...
def fetch_oembed_thumbnail(tiktok_url: str, timeout=15) -> str | None:
    """
    Fetch a fresh thumbnail URL from TikTok oEmbed API.
    Uses User-Agent header to avoid empty responses.
    ...
    try:
        ...
        #     logger.warning(
        #         "oEmbed returned status %s for %s. Response: %.200s",
        #         response.status_code, tiktok_url, response.text
        #     )
        #     return None
        ...
        if not thumbnail_url:
            logger.warning(
                "oEmbed missing thumbnail_url for %s. Response keys: %s",
                tiktok_url, list(data.keys())
            )
            return None

        logger.info("Fetched fresh thumbnail for %s", tiktok_url)
        return thumbnail_url

    except requests.exceptions.Timeout:
        logger.warning("oEmbed timeout for %s", tiktok_url)
        raise  # will trigger tenacity retry

    except requests.exceptions.RequestException as e:
        logger.warning("oEmbed request exception for %s: %s", tiktok_url, e)
        raise  # will trigger tenacity retry

    except (ValueError, json.JSONDecodeError) as e:
        logger.warning("oEmbed JSON decode error for %s: %s", tiktok_url, e)
        return None

    except Exception as e:
        logger.warning("Unexpected oEmbed error for %s: %s", tiktok_url, e)
        return None
...
def stage_openai_batches(**context):
    """
    Query Mongo for docs that don't have video_thumbnail_description OR video_thumbnail_keywords,
    ...
    old_input_files = [
        ...
        if "_output_" not in p.name and "_error_" not in p.name
    ]
    old_meta_files = list(TMP_DIR.glob("openai_batch_meta_*.json"))
    old_output_files = list(TMP_DIR.glob("openai_batch_output_*.jsonl"))
    old_error_files = list(TMP_DIR.glob("openai_batch_error_*.jsonl"))
    ...
    logger.info(
        "Cleanup complete: deleted %d input files, %d metadata files, %d output files, %d error files, progress file deleted=%s",
        deleted_input,
        deleted_meta,
        deleted_output,
        deleted_error,
    ...
    # Track statistics
    total_queried = 0
    skipped_no_username_or_id = 0
    skipped_no_thumbnail = 0
    processed = 0

    for doc in cursor:
        if processed >= MAX_THUMBNAIL_FETCH:
            logger.info(
                "Reached max fresh-thumbnail fetch cap (%d). Stopping staging early to avoid URL expiry.",
                MAX_THUMBNAIL_FETCH,
            )
            break
        ...
        if not username or not video_id:
            skipped_no_username_or_id += 1
            logger.debug("Skipping doc %s: missing username or video_id", doc.get("_id"))
            continue
        
        tiktok_url = f"https://www.tiktok.com/@{username}/video/{video_id}"
        
        # Always fetch fresh thumbnail URL (stored URLs may be expired/old)
        thumb = fetch_oembed_thumbnail(tiktok_url)
        
        # Small delay to avoid rate limiting
        time.sleep(0.5)
        
        # Only use fresh URLs - skip if oembed doesn't return one
        if not thumb:
            skipped_no_thumbnail += 1
            logger.info("Skipping (no fresh thumb available) %s", tiktok_url)
            continue
        
        processed += 1

        meta = {"_id": str(doc["_id"]), "video_url": tiktok_url, "thumbnail_url": thumb, "video_id": str(video_id)}
        buffer.append(build_openai_request_body(thumb, meta))
        metadata_buffer.append(meta)  # Store full metadata

        if len(buffer) >= CHUNK_SIZE:
            batch_num += 1
            timestamp = int(time.time())
            out = TMP_DIR / f"openai_batch_{batch_num}_{timestamp}.jsonl"
            meta_file = TMP_DIR / f"openai_batch_meta_{batch_num}_{timestamp}.json"
    ...
    logger.info("Processing summary: queried=%d, processed=%d, skipped_no_username_or_id=%d, skipped_no_thumbnail=%d", 
                total_queried, processed, skipped_no_username_or_id, skipped_no_thumbnail)
    # Also store metadata file paths for later use
    meta_file_paths = sorted(TMP_DIR.glob("openai_batch_meta_*.json"))
...
def submit_and_poll_batches(**context):
    ...
    for fp in file_paths:
        ...
        while True:
            ...
            try:
                if hasattr(batch_status, "request_counts"):
                    ...
                    # batch_status is a Pydantic object, not a dict, so we don't need the dict check
            ...
            if status in ("failed", "cancelled", "expired"):
                ...
                try:
                    ...
                    # batch_status is a Pydantic object, not a dict
                    if request_counts:
        ...
        if output_file_id:
            ...
            outpath = TMP_DIR / f"openai_batch_output_{batch_id}.jsonl"
        ...
        if error_file_id:
            ...
            error_outpath = TMP_DIR / f"openai_batch_error_{batch_id}.jsonl"
        ...
        # Input file: openai_batch_{batch_num}_{timestamp}.jsonl
        # Extract timestamp from input file name
        input_path = Path(fp)
        input_stem = input_path.stem  # e.g., "openai_batch_1_1764283236"
        # Try to find matching metadata file
        meta_file_path = None
        if "_" in input_stem:
            parts = input_stem.split("_")
            if len(parts) >= 4:  # openai_batch_{num}_{timestamp}
                batch_num = parts[2]
                timestamp = parts[3]
                meta_pattern = f"openai_batch_meta_{batch_num}_{timestamp}.json"
...
def parse_outputs_and_update(**context):
    ...
    # If Op1 was never run or the worker restarted, rebuild from progress first.
    if not submitted_info:
        ...
        for _, record in completed_map.items():
            ...
            if (
                ...
                and (not meta_path or Path(meta_path).exists())
            ):
                submitted_info.append(record)
        # Last fallback if progress file is missing: scan output files.
        if not submitted_info:
            logger.warning("No progress records found — scanning %s for output files.", TMP_DIR)
            for p in sorted(TMP_DIR.glob("openai_batch_output_*.jsonl")):
                submitted_info.append({
                    "output_path": str(p),
                    "batch_id": p.stem.replace("openai_batch_output_", "")
                })
    ...
    for batch in submitted_info:
        ...
        if parse_key and parsed_outputs.get(parse_key, {}).get("status") == "completed":
            logger.info("Skipping already parsed batch (key=%s)", parse_key)
            continue
        if not fpath.exists():
        ...
        batch_id = batch.get("batch_id") or fpath.stem.replace("openai_batch_output_", "")
        ...
        if not metadata_map:
            meta_files = sorted(TMP_DIR.glob("openai_batch_meta_*.json"))
            if meta_files:
        ...
        if not metadata_map:
            logger.warning("No metadata mapping file found for batch %s - video_url/thumbnail_url may be missing", batch_id)
        ...
        with fpath.open("r", encoding="utf-8") as fh:
            for line in fh:
                ...
                thumb_url = full_meta.get("thumbnail_url")
                ...
                if desc is not None:
                    update["video_thumbnail_description"] = desc
                if kw is not None:
                    update["video_thumbnail_keywords"] = kw
                if video_url:
                    update["video_url"] = video_url
                if thumb_url:
                    update["video_thumbnail_url"] = thumb_url
                ...
                with neo4j.session() as session:
                    ...
                    try:
                        ...
                        if sample_nodes:
                            ...
                            logger.warning("No TikTokVideo nodes found in Neo4j - cannot update")
                    except Exception as e:
                    ...
                    for i in range(0, len(valid_rows), NEO4J_CHUNK):
                        ...
                        SET v.video_thumbnail_description = r.description,
                            v.video_thumbnail_keywords = r.keywords,
        ...
        if parse_key:
            parsed_outputs[parse_key] = {
                ...
                "metadata_file": batch.get("metadata_file"),
