...
CSV_FILE_PATH = os.path.join(SCRIPT_DIR, "area_nodes.csv")
...
def validate_csv_data(df):
    ...
    # Check for empty area names
    empty_area_names = df["area_name"].isna() | (df["area_name"].str.strip() == "")
    if empty_area_names.any():
        empty_indices = df[empty_area_names].index.tolist()
        raise ValueError(f"Empty area names found at rows: {empty_indices}")
    ...
    print(f"CSV validation passed. Found {len(df)} valid records.")
...
def main():
    """Main function to import area nodes to Neo4j."""
    try:
        ...
        # Query template
        query = """
        MERGE (a:Area {area_name: $area_name})
        SET a += $props
        """
            ...
            with session.begin_transaction() as tx:
                ...
                for index, row in df.iterrows():
                    try:
                        ...
                        # Convert all values properly, skip area_name in props
                        props = {k: convert_value(v) for k, v in row_dict.items() if k != "area_name"}
                        
                        tx.run(query, area_name=row["area_name"], props=props)
                        success_count += 1
                        
                    except Exception as e:
                        print(f"Error processing row {index} ({row.get('area_name', 'Unknown')}): {e}")
                        continue
        
        print(f"Successfully imported {success_count} out of {len(df)} Area nodes to Neo4j!")
